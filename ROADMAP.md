...
- **Format support is narrower than the original hackathon corpus**: only `.docx` (via `mammoth`) and `.pdf` (via `unpdf`) are supported. **PPTX is NOT supported** — no lightweight, well-maintained pure-JS PPTX text extractor was found; the one broad-format option (`officeparser`) bundles `tesseract.js` (a full OCR/WASM engine) as a hard dependency, too heavy for a serverless function. This matters because the actual source files this school's content came from *are* PPTX (see the header comment in `src/data/corpus.ts`) — a teacher re-uploading the original hackathon material today would need to convert it to `.docx`/`.pdf` first. Revisit if a suitable PPTX library appears, or budget for a dedicated conversion step.
- **No class-management UI exists**, so `src/lib/ingestion/class.ts`'s `getOrCreateTeacherClass()` auto-provisions one class per teacher on their first upload (subject/grade taken from the upload form). Same single-pilot-school simplification as `DEFAULT_SCHOOL_ID` — reasonable for one school's one class per teacher, not a real roster system. This also means the `class_id` blocker noted above (conversations/conversation_turns) is now *structurally* resolved by this task, even though wiring conversation logging itself remains out of scope here.
